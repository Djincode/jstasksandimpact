...
let counter = document.querySelector(".product__counter input");
...
constructor(){
  ...
  this.productCount =1;
  this.cart = [];
  this.fetchProductIdFromStorage();
  this.displayProductInfo()
  this.fetchChartFromStorage()
...
increaseCount(){
  ...
  counter.value = this.productCount;

}
decreaseCount(){
  this.productCount>1 && this.productCount--;
  counter.value = this.productCount;
}
addToCart(){
  const existingProduct = this.cart.find((product)=>product.id === this.product.id);
  const existingProductIndex= this.cart.findIndex((product)=> product.id === this.product.id);
  if(!existingProduct){
   const newProduct = {
   ...
   this.cart.push(newProduct);
    ...
    const addedProduct = {...existingProduct};
    addedProduct.quantity += this.productCount;
    this.cart.splice(existingProductIndex,1,addedProduct);
